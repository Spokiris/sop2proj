...
int ems_show(int out_fd, unsigned int event_id) {
  ...
  if (write(out_fd, &event->rows, sizeof(size_t)) != sizeof(size_t) ||
    write(out_fd, &event->cols, sizeof(size_t)) != sizeof(size_t)) {
    perror("Error writing to file descriptor");
    pthread_mutex_unlock(&event->mutex);
    return 1;
  }

  // Escreve o array de seats
  for (size_t i = 1; i <= event->rows; i++) {
    for (size_t j = 1; j <= event->cols; j++) {
      unsigned int seat = event->data[seat_index(event, i, j)];
      if (write(out_fd, &seat, sizeof(unsigned int)) != sizeof(unsigned int)) {
        perror("Error writing to file descriptor");
        pthread_mutex_unlock(&event->mutex);
        return 1;
      }
...
int ems_list_events(int out_fd) {
  ...
  size_t num_events = 0;
  for (struct ListNode* node = current; node != NULL; node = node->next) {
    num_events++;
  }
  ...
  // Preenche o array com os IDs dos eventos
  size_t i = 0;
  while (current != NULL) {
  ...
  // Escreve o número de eventos e o array de IDs
  if (write(out_fd, &num_events, sizeof(size_t)) != sizeof(size_t) ||
      write(out_fd, ids, num_events * sizeof(unsigned int)) != num_events * sizeof(unsigned int)) {
    perror("Error writing to file descriptor");
    free(ids);
    pthread_rwlock_unlock(&event_list->rwl);
    return 1;
